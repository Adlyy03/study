                ...
                <div class="flex items-center xl:p-10">
                    <form method="POST" action="{{ route('login') }}" class="flex flex-col w-full h-full pb-6 text-center bg-white rounded-3xl">
                        ...
                        <div class="text-left w-full mb-4">
                            <x-input-label for="email" :value="__('Email')" class="mb-2 text-sm text-grey-900"/>
                            <x-text-input id="email" type="email" name="email" :value="old('email')" required autofocus
                                placeholder="mail@loopple.com"
                                class="flex items-center w-full px-5 py-4 text-sm font-medium outline-none focus:bg-grey-400 placeholder:text-grey-700 bg-grey-200 text-dark-grey-900 rounded-2xl"/>
                            <x-input-error :messages="$errors->get('email')" class="mt-2" />
                        </div>

                        <!-- Password -->
                        <div class="text-left w-full mb-4">
                            <x-input-label for="password" :value="__('Password')" class="mb-2 text-sm text-grey-900"/>
                        ...
                        <div class="flex flex-row justify-between mb-8">
                            ...
                            @if (Route::has('password.request'))
                                <a class="text-sm font-medium text-purple-blue-500" href="{{ route('password.request') }}">
                        ...
                        <!-- Submit -->
                        <x-primary-button class="w-full px-6 py-5 mb-5 text-sm font-bold leading-none text-white transition duration-300 md:w-96 rounded-2xl hover:bg-purple-blue-600 focus:ring-4 focus:ring-purple-blue-100 bg-purple-blue-500">
                            {{ __('Sign In') }}
                        </x-primary-button>
                        ...
                        <p class="text-sm leading-relaxed text-grey-900">
                            Not registered yet?
                            <a href="{{ route('register') }}" class="font-bold text-grey-700">Create an Account</a>
                        </p>
                    </form>
                </div>
            </div>
